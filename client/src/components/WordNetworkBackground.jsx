...
const WordNetworkBackground = ({ gamePage = false }) => {
  ...
  useEffect(() => {
    ...
    // Créer un dégradé en arrière-plan
    const createGradient = () => {
      // Utiliser un dégradé diagonal
      ...
      if (gamePage) {
        ...
        gradient.addColorStop(0, "rgba(198, 214, 255, 0.97)");
        gradient.addColorStop(0.3, "rgba(179, 197, 255, 0.97)");
        gradient.addColorStop(0.7, "rgba(137, 154, 231, 0.97)");
        gradient.addColorStop(1, "rgba(109, 130, 220, 0.97)");
      } else {
        // Dégradé pour la page d'accueil - tons violet/indigo
        gradient.addColorStop(0, "rgba(220, 227, 255, 0.97)");
        gradient.addColorStop(0.3, "rgba(192, 203, 255, 0.97)");
        gradient.addColorStop(0.7, "rgba(164, 174, 231, 0.97)");
        gradient.addColorStop(1, "rgba(126, 140, 215, 0.97)");
      }
    ...
    const createParticles = () => {
      ...
      const wordleWords = gamePage
        ? ["LETTRES", "DEVINE", "INDICE", "SCORE", "VICTOIRE"]
        : ["WORDLE", "LETTRES", "DEVINE", "JOUER", "MOTS"];
      ...
      for (let i = 0; i < particleCount; i++) {
        particles.push({
          ...
          angle: Math.random() * Math.PI * 2,
          color: gamePage
            ? `rgba(130, 140, 220, 0.75)`
            : `rgba(125, 135, 240, 0.75)`,
          visible: true,
          isPoint: true,
    ...
    const createConnections = () => {
      ...
      const connectionThreshold = gamePage ? 0.75 : 0.7;
        ...
        for (let j = i + 1; j < particles.length; j++) {
          if (Math.random() > connectionThreshold) {
            connections.push({
              from: i,
              to: j,
              opacity: Math.random() * 0.4 + 0.15, // Opacité aléatoire pour les connexions
            });
          }
        }
      }
    ...
    const animate = () => {
      ...
      connections.forEach((connection) => {
        ...
        const maxDistance =
          Math.min(canvas.width, canvas.height) * (gamePage ? 0.35 : 0.4);
        if (distance < maxDistance) {
          ctx.beginPath();
          ctx.moveTo(p1.x, p1.y);
          ctx.lineTo(p2.x, p2.y);
          ctx.strokeStyle = gamePage
            ? `rgba(120, 140, 230, ${connection.opacity})`
            : `rgba(130, 145, 250, ${connection.opacity})`;
          ctx.lineWidth = 1;
          ctx.stroke();
        }
      });
      ...
      particles.forEach((particle) => {
        ...
        const speedMultiplier = gamePage ? 0.5 : 0.6;
        particle.x +=
        ...
        if (particle.isPoint) {
          ...
          ctx.fillRect(particle.x - 65, particle.y - 18, 130, 30);

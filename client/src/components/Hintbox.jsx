...
// Composant qui affiche les indices disponibles pour aider le joueur
const HintBox = ({ targetWord, onHintUsed, position = "inline" }) => {
  ...
  // Réinitialise les indices quand le mot cible change
  useEffect(() => {
    if (targetWord !== previousWord) {
      ...
      if (targetWord) {
        ...
        // Sélection aléatoire de 3 indices parmi tous les disponibles
        const selectedHints = [];
  ...
  // Génère tous les types d'indices possibles pour le mot
  const generateAllHints = (word) => {
    ...
    const allHints = [
      ...
      // Nombre approximatif de syllabes
      (() => {
        const vowels = ["A", "E", "I", "O", "U", "Y"];
        let syllables = 0;
        ...
        return `Ce mot contient environ ${syllables} syllabe${
          syllables > 1 ? "s" : ""
        }`;
      })(),
    ];
  ...
  // Gère la révélation d'un indice et met à jour le compteur
  const revealHint = (index) => {
    if (!hintsRevealed[index]) {
      ...
      // Nombre total d'indices révélés
      const hintCount = newHintsRevealed.filter(Boolean).length;

      // Notifie le parent pour ajuster le score
      if (onHintUsed) {
  ...
  return (
    <div>
      <div className="bg-white/80 backdrop-blur-sm p-4 rounded-lg shadow-lg border-l-4 border-green-500 animate-fadeIn max-w-xs">
            ...
            >
              ...
              {hintsRevealed[index] ? (
                ...
                <p className="text-sm text-gray-400 italic mt-1">
                  Cliquez pour révéler cet indice
                </p>
              )}
            </div>
          ))}
